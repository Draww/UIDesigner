...
public class ReflectionUtil {
	 ...
	 */
	public static void updateInventoryTitle(Player pl, String title) {
		try {
			if (SERVER_VERSION_NUM < 8)
				return;

			if (SERVER_VERSION_NUM < 9 && title.length() > 16)
			...
			final Object chatMessage = chatMessageConst.newInstance(DesignerUtils.colorize(title), new Object[0]);

			final Object packet = packetConst.newInstance( windowId, "minecraft:chest", chatMessage, pl.getOpenInventory().getTopInventory().getSize() );
			...
			System.out.println("Error updating " + pl.getName() + " inventory title to '" + title + "'");
	 ...
	 */
	public static final String bountify(Enum<?> e) {
		return WordUtils.capitalizeFully(e.name().toLowerCase().replace("_", " "));
	}

	/**
	 * Automatically recognizes the clicked inventory (can either be the top or the bottom).
	 */
	...
	/**
	 * An exception thrown when the enum is missing.
	 */
	...
	/**
	 * An exception thrown when an error in reflection occur.
	 */
